...
// Заполнение с консоли (для лаб 11)
...
// Вставить значение на позицию (без realloc, с новым malloc для безопасности)
double* insert_value_at(double* arr, int* size, double value, int pos) {
    ...
    if (!new_arr) {
        ...
        return arr;  // Возвращаем старый, size не меняем
    }
...
// Печать массива (реализация-дубликат, но существует в header)
int put_array_int(int* ptr_array, int n) {
